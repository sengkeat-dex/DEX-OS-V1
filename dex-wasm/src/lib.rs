...
use dex_core::{orderbook::OrderBook, amm::ConstantProductAMM, types::Order};
    ...
    pub fn add_order(&mut self, order: JsValue) -> Result<JsValue, JsValue> {
        ...
        match self.inner.add_order(order) {
            Ok(trades) => {
                ...
                let js_trades = serde_wasm_bindgen::to_value(&trades)
                    .map_err(|e| JsValue::from_str(&format!("Failed to serialize trades: {}", e)))?;
                Ok(js_trades)
            }
...
impl WasmAMM {
    ...
    #[wasm_bindgen]
    pub fn add_liquidity(&mut self, token_a: String, amount_a: u64, token_b: String, amount_b: u64) -> Result<u64, JsValue> {
        self.inner.add_liquidity(token_a, amount_a, token_b, amount_b)
            .map_err(|e| JsValue::from_str(&format!("Failed to add liquidity: {}", e)))
    }

    /// Remove liquidity from the pool
    #[wasm_bindgen]
    pub fn remove_liquidity(&mut self, token_a: String, token_b: String, liquidity_tokens: u64) -> Result<JsValue, JsValue> {
        match self.inner.remove_liquidity(token_a, token_b, liquidity_tokens) {
            ...
            Err(e) => Err(JsValue::from_str(&format!("Failed to remove liquidity: {}", e)))
        }
    }

    /// Swap tokens in the pool
    #[wasm_bindgen]
    pub fn swap(&mut self, from_token: String, to_token: String, amount_in: u64) -> Result<u64, JsValue> {
        self.inner.swap(from_token, to_token, amount_in)
    ...
    pub fn get_price(&self, from_token: String, to_token: String) -> Result<f64, JsValue> {
        self.inner.get_price(&from_token, &to_token)
            .map_err(|e| JsValue::from_str(&format!("Failed to get price: {}", e)))
    }

    /// Find the optimal price within a given range using binary search
    #[wasm_bindgen]
    pub fn find_price_in_range(&self, from_token: String, to_token: String, min_price: f64, max_price: f64, tolerance: f64) -> Result<f64, JsValue> {
        self.inner.find_price_in_range(&from_token, &to_token, min_price, max_price, tolerance)
    ...
    #[wasm_bindgen]
    pub fn is_price_within_slippage(&self, from_token: String, to_token: String, proposed_price: f64, max_slippage: f64) -> Result<bool, JsValue> {
        self.inner.is_price_within_slippage(&from_token, &to_token, proposed_price, max_slippage)
            .map_err(|e| JsValue::from_str(&format!("Failed to check price within slippage: {}", e)))
    }
}
