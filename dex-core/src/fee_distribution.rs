...
use crate::types::{TraderId, Quantity, TokenId};
...
impl FeeDistributionManager {
    ...
    pub fn add_distribution(&mut self, distribution: FeeDistribution) {
        ...
        self.distributions.insert(distribution.trader_id.clone(), distribution);
    }
    ...
    pub fn update_distribution_amount(&mut self, trader_id: &TraderId, new_amount: Quantity) -> Result<(), FeeDistributionError> {
    ...
    pub fn get_traders_in_range(&self, start: &TraderId, end: &TraderId) -> Vec<&FeeDistribution> {
        self.distributions
            .range::<std::string::String, _>((std::ops::Bound::Included(start), std::ops::Bound::Included(end)))
            .map(|(_, dist)| dist)
            .collect()
    }
...
mod tests {
    ...
    fn test_remove_distributions() {
        ...
        assert!(manager.remove_distribution(&"trader2".to_string()).is_none());
    }
    ...
    fn test_update_distribution_amount() {
        ...
        // Update the amount
        assert!(manager.update_distribution_amount(&"trader1".to_string(), 1500).is_ok());
        ...
        assert!(manager.update_distribution_amount(&"trader2".to_string(), 2000).is_err());
    }
    ...
    fn test_get_traders_in_range() {
        ...
        let ranged_distributions = manager.get_traders_in_range(
            &"trader2".to_string(),
            &"trader4".to_string()
        );
